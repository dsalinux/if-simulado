...
import br.edu.ifnmg.arinos.util.HibernateUtil;
...
public abstract class AbstractManager extends GenericBean {
    ...
    public final <T> T doInTransaction(PersistenceAction<T> action) throws BusinessException {
    ...
    public final void doInTransaction(PersistenceActionWithoutResult action) throws BusinessException {
        Transaction transaction = HibernateUtil.getSession().getTransaction();
        if(transaction == null){
            transaction = HibernateUtil.getSession().beginTransaction();
        } else if(!transaction.isActive()){
